...
const elements = {
    ...
    // Sidebar & Header
    currentUserPic: el('currentUserPic'),
    sidebar: el('sidebar'),
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 2. Carregar Dados
    await loadDeckDetails(deckId, currentUser);
    ...
    try {
        ...
        if (deck.profiles) {
            ...
            elements.creatorName.onclick = () => {
                window.location.href = `../profile/viewProfile.html?id=${deck.user_id}`;
            };
        }
...
function initSidebar() {
    const toggle = (s) => {
        if(s) { elements.sidebar?.classList.add('active'); elements.overlay?.classList.add('active'); }
        else { elements.sidebar?.classList.remove('active'); elements.overlay?.classList.remove('active'); }
    };
